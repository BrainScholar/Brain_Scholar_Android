        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i < 10000; ) {
                    ...
                    f[i%5000] = v[i%5000] * (1 - ((v[i%5000] * v[i%5000]) / 3));
                    v[(i + 1)%5000] = 1 / c * (gna * f[i%5000] - gk * u[i%5000]) * del_t + v[i%5000];
                    if (intstim.equals(i)) {
                        v[(i + 1)%5000] = v[(i + 1)%5000] + v_stim;
                    }
                    u[(i + 1)%5000] = (v[i%5000] + beta - gamma * u[i%5000]) * del_t + u[i%5000];
                    runOnUiThread(new Runnable() {

                        @Override
                        public void run() {
                            series.appendData(new DataPoint(iteration, v[iteration%5000]), true, 100000);
